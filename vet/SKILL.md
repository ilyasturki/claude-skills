...
2. **Pick the right test harness** based on what changed — do NOT rely solely on unit tests or type checks, those verify code correctness, not feature correctness:
   ...
   - **Browser** playwright for web UI changes — navigate, click, fill forms, watch console, verify the UI responds correctly. Check if there is no errors in the console or in the page.
